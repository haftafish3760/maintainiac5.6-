...
Additional OCR source count-map fields in the same redacted set:
`ocrSourceCoverageSignalCounts`, `ocrSourceContinuationSignalCounts`,
`ocrSourceSectionOrderSignalCounts`, `ocrSourceSectionOrderReviewStatusCounts`,
and `ocrSourceSectionOrderFailedPairStatusCounts`.

Native camera readiness count-map fields in the same redacted set:
