...
- Verified 2026-07-23 evidence: 30 maintenance persistence/draft/UI/history
  tests, 69 selected vehicle/global-odometer tests, and the 184-check maintenance
  receipt gate passed. The receipt gate includes 178 parser/review/draft/UI/
...
`NO CHARGE` alone. Real multi-operation invoice validation, labor/part grouping,
and broader warranty/comeback coverage remain open.
...
`docs/maintenance_receipt_parser_pass_log.md`. The current accepted pass is 51;
the next bundled app-work pass is 52. Duplicate-line cleanup remains OCR-owned,
not parser-owned.
