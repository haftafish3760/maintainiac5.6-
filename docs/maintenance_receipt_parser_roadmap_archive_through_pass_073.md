# Maintenance Receipt Parser Roadmap Archive Through Pass 73
...
- Verified 2026-07-23 evidence: 30 maintenance persistence/draft/UI/history
  tests, 69 selected vehicle/global-odometer tests, and the 213-check maintenance
  receipt gate passed. The receipt gate includes 207 parser/review/draft/UI/
...
- If the receipt contains no interval, review uses the matching manual catalog
  defaults as editable app suggestions. Printed or safely derived receipt
  schedules take precedence, and an explicit user clear remains authoritative.
  These defaults are not presented as vehicle/OEM facts or inferred from oil
  viscosity.
- Setup tracking requires at least one positive mileage or time interval.
  Time-only catalog items omit their non-applicable zero-mile value, and every
  review entry path uses the same interval-prefilled item factory. Setup-only
  records remain pending until the user supplies a last-service baseline;
  confirmed service evidence completes that baseline without changing the
  canonical vehicle odometer.
- Receipt-applied records preserve the matching manual catalog's importance
  and time-only semantics. This keeps time-based maintenance such as
  registration and inspection out of mileage-based due calculations while
  retaining the same catalog behavior as manual setup.
- Older version-one receipt-review drafts with absent, unedited interval fields
  migrate to the same catalog suggestions on restore. Explicitly cleared
  fields remain cleared, preserving user intent across restart.
- Completed time-only receipt services require a confirmed date but do not
  invent odometer evidence. Date-only duplicate scans are blocked by stable
  vehicle, item, and service date. Mileage-based service still requires a
  confirmed odometer, and receipt application never mutates canonical odometer
  state.
- Missing service odometer evidence is displayed as “Not recorded” in
  maintenance history, service detail, and Calendar projection rather than as
  a fabricated zero-mile reading. Positive readings retain mileage display.
- The manual service form follows the same evidence rule: it seeds the shared
  odometer for mileage-based maintenance, but a blank time-only visit remains
  “Not entered” and saves no odometer evidence. This keeps the manual-first and
  receipt-assisted flows semantically aligned.
- Mixed manual visits retain odometer provenance. Removing the last
  mileage-based item clears a still-automatic seed, while user-entered and
  restored-draft readings remain visible and reviewable.
- Receipt review labels interval provenance as receipt evidence, editable app
  suggestion, user-reviewed values, or absent. Both receipt-derived and generic
  schedules direct the user to confirm against the selected vehicle or OEM
  schedule; oil type/weight and merchant identity do not determine fallback.
...
heading resets that state. Standalone customer-refusal, not-authorized/
not-approved, cancelled-service/work/repair, and no-work-performed headings are
also scoped rather than downgrading unrelated earlier work. Technician notes/
comments, observations, advisories, and vehicle-health report/result headings
now provide the same review-only boundary without using merchant layouts.
...
## Quiet QA loop for each maintenance pass

1. Declare one coherent file or file-group scope and inspect existing coverage.
2. Extend or parameterize coverage when possible; add a test only for a
   genuinely uncovered behavior, with no more than five without approval.
3. Keep every maintenance receipt source/test/gate file below 500 lines.
4. Complete the scoped edits, then run changed-file analysis and direct Tier 1
   tests until green. Run Tier 2 after feature/shared-contract batches or every
   10 passes, Tier 3 at checkpoints or every 50 passes, and Tier 4 for release.
5. Record the eight-part pass report, including before/after test counts and
   remaining risk. Only then close the group and increment the pass once.
...
`docs/maintenance_receipt_parser_pass_log.md`. The current accepted pass is 73;
the next bundled app-work pass is 74. Duplicate-line cleanup remains OCR-owned,
not parser-owned.
