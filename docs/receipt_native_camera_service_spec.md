...
- Native capture results now carry camera evidence instead of a placeholder: engine, capture surface, receipt modes, torch, brightness/exposure, zoom, original photo byte size, and the fact that OCR uses the original first.
...
- **Native Pass 31:** Add native long-receipt batch capture to CameraX/AVFoundation so long receipt mode keeps the camera open after each section, shows a section-count Done action, returns all ordered original photo paths through the bridge, and reports photo count/max section diagnostics. **Complete.**
