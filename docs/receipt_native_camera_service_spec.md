...
- **Native Pass 36:** Keep throttled native live brightness/readability analysis available for CameraX and AVFoundation, but treat dark/glare/over-bright warnings as experimental quality guidance that stays default-off unless separately enabled and proven. Manual capture stays available and no preview image content is stored. **Complete.**
...
- **Native Pass 41:** Keep nonblocking live steadiness/motion detection available for CameraX/AVFoundation, but gate user-facing steadiness warnings behind experimental receipt-quality guidance. Default release-one guidance remains neutral receipt workflow/framing guidance, with only safe motion signal/score stored in diagnostics. **Complete.**
- **Native Pass 42:** Expose receipt guidance warnings in native Android/iOS settings, separating neutral receipt workflow/framing warnings from experimental shake/light/glare/shadow/dirty-lens quality warnings so manual shutter, focus, zoom, brightness, and app-assisted receipt review keep working. **Complete.**
