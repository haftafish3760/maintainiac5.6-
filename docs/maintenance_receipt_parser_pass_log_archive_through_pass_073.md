# Maintenance Receipt Parser Pass Log Archive Through Pass 73
...
63. Merchant-neutral advisory narrative sections. Technician notes/comments,
    observations, advisories, and vehicle-health report/result headings now
    end inherited completed-service state, so a reported condition cannot be
    presented as performed work. Three superficial wording/layout variants
    were consolidated into one parameterized Tier 1 contract rather than
    three separate tests. Changed-file analysis and 15 directly affected
    section contracts passed. No interface, persistence, state, serialization,
    navigation, or platform boundary changed, so Tier 2 and Tier 3 were not
    required; the latest full checkpoint remains Pass 62's 213-check gate.
64. Receipt-review interval fallback and precedence. When receipt evidence has
    no mileage or time interval, the review now supplies the matching manual
    maintenance catalog defaults as editable Basic suggestions. A printed or
    derived receipt schedule remains authoritative, and an explicit user clear
    remains clear. This prevents receipt-assisted setup from degrading into an
    unusable zero-mile/zero-month record without claiming that oil type or
    viscosity determines the interval. Production files changed:
    `maintenance_receipt_review.dart` and
    `maintenance_receipt_review_commands.dart`. One existing review test was
    extended; no test functions were added, removed, or consolidated. Tier 1
    changed-file analysis and all 12 review tests passed. Tier 2 was required
    because confirmed commands feed review UI, drafts, and application; all 32
    affected review/application/dialog/duplicate/draft/widget tests passed.
    Tier 3 was not due. Remaining risk: these catalog defaults are editable app
    suggestions, not vehicle/OEM-specific schedules, and need clear provenance
    in the eventual final UX. Total maintenance receipt checks remain 214
    before and after this pass; the latest full checkpoint remains Pass 62's
    213-check gate.
65. Valid Basic interval contract across review entry paths. Setup tracking now
    requires at least one positive mileage or time interval, while service-only
    logging may omit both. Catalog distance value `0` is translated to no
    mileage interval for time-only items, so registration/inspection-style
    maintenance can retain its month interval without application rejecting a
    non-applicable zero-mile payload. Review-item construction is centralized
    so direct screen entry receives the same defaults as the review factory.
    Production files changed: `maintenance_receipt_review_commands.dart` and
    `maintenance_receipt_review_screen.dart`. Two existing tests were extended;
    no test functions were added, removed, or consolidated. Tier 1 analysis and
    21 direct review/application tests passed. The first combined Tier 2 run
    exposed the screen-entry bypass and then stalled in its waiting widget; it
    was stopped, corrected, and not counted green. The isolated failed widget
    passed, followed by 25 non-screen and seven screen Tier 2 tests. Tier 3 was
    not due. Setup-only records intentionally remain incomplete until the user
    supplies a last-service baseline; a confirmed service event completes
    them. Total maintenance receipt checks remain 214 before and after this
    pass; the latest full checkpoint remains Pass 62's 213-check gate.
66. Receipt-applied catalog metadata parity. Receipt-created setup records now
    preserve the matched manual catalog's importance and time-only flag instead
    of hard-coding importance 100 and mileage-based behavior. Registration is
    proven end to end with no mileage interval, 12 months, importance 98, and
    `timeOnly: true`. Production files changed:
    `maintenance_receipt_review_commands.dart` and
    `maintenance_receipt_application_service.dart`. Two existing tests were
    modified; no test functions were added, removed, or consolidated. Tier 1
    analysis and all nine application tests passed. Tier 2 persistence and
    calendar projection checks passed, while the maintenance tracking file
    exposed a pre-existing partial-text assertion that matched both the exact
    price and its recap sentence. Its isolated rerun failed identically, so the
    assertion was tightened to the full recap contract; the isolated test and
    all nine tracking tests then passed. Tier 2 finished with 19 green checks.
    Tier 3 was not due. Unknown/non-catalog manual-review items retain safe
    generic metadata and require review. Total maintenance receipt checks
    remain 214 before and after this pass; the latest full checkpoint remains
    Pass 62's 213-check gate.
67. Legacy receipt-review interval migration. Version-one drafts that predate
    catalog interval prefills now restore missing, unedited mileage/time values
    from the matching catalog, while an explicit cleared-field marker remains
    authoritative. This lets an older local review resume setup after restart
    without changing user intent. Production file changed:
    `maintenance_receipt_review.dart`. The existing schema-rejection test moved
    from the 498-line general review file into the already harness-registered
    draft-store test, and one distinct migration test was added there; no test
    was removed. The shareable QA harness was not changed. Current-file Tier 1
    analysis, 11 review tests, and six draft-store/compatibility tests passed.
    Tier 2 was required for serialization/lifecycle impact: the affected set
    totaled 15 green draft-store, draft-resume, compatibility, and isolated
    review-screen checks. Tier 3 was not due. Remaining risk: future structural
    draft changes still require an explicit schema version migration. Total
    maintenance receipt checks increased from 214 to 215; the latest full
    checkpoint remains Pass 62's 213-check gate.
68. Completed time-only service without invented odometer evidence. Receipt
    application now requires service odometer evidence only for mileage-based
    catalog items. Registration/inspection-style completed services require a
    confirmed date, use the established zero sentinel for “not recorded,” and
    can complete their time-based setup without changing canonical odometer
    state. Different scans of the same date-only service are blocked by stable
    vehicle, item, and service date even when no odometer was supplied.
    Production file changed: `maintenance_receipt_application_service.dart`.
    Two existing tests were extended; no test functions were added, removed,
    or consolidated. Tier 1 analysis and the final 10 application/duplicate
    checks passed. Tier 2 was required for event, duplicate, persistence, and
    downstream projection impact: eight persistence, one calendar projection,
    one duplicate, and seven isolated review-screen checks passed. An initial
    invalid test assertion tried to read odometer from `VehicleProfile`; it
    failed compilation, was removed because canonical odometer has a separate
    owner, and the tier was rerun green. Tier 3 was not due. Remaining risk:
    downstream history/detail/calendar presentation still renders sentinel
    zero as `0 miles` instead of “Not recorded.” Total maintenance receipt
    checks remain 215 before and after this pass; the latest full checkpoint
    remains Pass 62's 213-check gate.
69. Missing-odometer presentation fidelity. Maintenance history rows, service
    record detail, and Calendar maintenance projection now present the
    established zero sentinel as “Odometer not recorded”/“Not recorded” rather
    than claiming the service occurred at zero miles. Positive odometers retain
    their existing mileage display. Production files changed:
    `maintenance_service_history.dart`,
    `maintenance_service_event_detail_screen.dart`, and
    `calendar_maintenance_projection_adapter.dart`. Two existing tests were
    modified; no test functions were added, removed, or consolidated. Tier 1
    analysis plus the existing Calendar projection, normal-odometer Calendar
    widget, and missing-odometer history/detail widget contracts passed. The
    first history run failed because the mechanical fixture edit targeted the
    earlier Calendar event; the two values were corrected explicitly and all
    affected tests were rerun green. Tier 2 was required for upstream event
    application/persistence and downstream UI impact: nine application, eight
    persistence, one projection, and nine isolated maintenance tracking tests
    passed (27 total). Tier 3 was not due. Remaining risk: the manual time-only
    service form currently seeds the live odometer even though its helper says
    odometer is optional; receipt application itself no longer invents it.
    Total maintenance receipt checks remain 215 before and after this pass; the
    latest full checkpoint remains Pass 62's 213-check gate.
70. Manual time-only service parity and scheduled Tier 2 checkpoint. The
    foundational manual service form now seeds the shared odometer only when
    the selected maintenance contains a mileage-based item. A blank odometer
    for registration/inspection-style time-only service remains “Not entered”
    through review and saves the established not-recorded sentinel instead of
    silently copying the current vehicle reading. Mileage-based service still
    seeds and saves the canonical shared reading. Production file changed:
    `maintenance_log_service_screen.dart`. One existing widget test was
    extended to cover both branches in the same test function; no tests were
    added, removed, or consolidated. Tier 1 analysis and the direct manual-log
    widget contract passed. Pass 70 required the scheduled Tier 2 checkpoint:
    30 local-store/persistence/draft/projection/receipt-application checks,
    four archive/resume widgets, and nine isolated maintenance tracking widgets
    passed (43 total). Tier 3 was not due. Remaining risk: if a mixed selection
    auto-seeds odometer and the user later removes every mileage-based item, the
    visible value remains until the user clears it; it is not hidden and still
    passes through review. Total maintenance receipt checks remain 215 before
    and after this pass; the latest full checkpoint remains Pass 62's 213-check
    gate.
71. Mixed-selection odometer provenance. The manual service flow now tracks
    whether its odometer value was programmatically seeded. Removing the final
    mileage-based item from a mixed visit clears that value only while it is
    still automatic; any user edit or restored draft value removes the
    auto-seeded marker and is preserved for visible review. Re-adding a
    mileage-based item can seed the canonical reading again. Production file
    changed: `maintenance_log_service_screen.dart`. One existing widget test
    was extended to exercise mileage selection, time-only addition, mileage
    removal, review, and zero-evidence save in the same test function; no test
    was added, removed, or consolidated. Tier 1 analysis and the direct widget
    contract passed after the test was corrected to pump rebuilt selection
    widgets between taps. Tier 2 was required for selection/draft lifecycle
    impact: eight draft store/resume checks and all nine isolated maintenance
    tracking widgets passed (17 total). Tier 3 was not due. Remaining risk:
    `maintenance_tracking_selection_test.dart` is now 498 lines and must be
    split before future expansion. Total maintenance receipt checks remain 215
    before and after this pass; the latest full checkpoint remains Pass 62's
    213-check gate.
72. Explicit interval provenance in receipt review. Review items now classify
    the visible schedule as receipt evidence, an editable app suggestion,
    user-reviewed values, or absent. Basic and Advanced review surfaces show
    that source directly below the interval fields. Receipt-derived schedules
    are still presented as evidence to confirm against the selected vehicle or
    OEM schedule; catalog defaults explicitly state that the receipt supplied
    no interval and do not imply that merchant, oil type, or viscosity selected
    the schedule. Production files changed: `maintenance_receipt_review.dart`
    and `maintenance_receipt_review_item_card.dart`. Existing review-model and
    review-screen tests were extended; no test was added, removed, or
    consolidated. Tier 1 analysis, all 11 review-model contracts, and focused
    purchase/service widgets passed. Initial widget assertions ran while items
    were still undecided and correctly found no interval UI; they were moved
    after recommendation acceptance and rerun green. Tier 2 was required for
    review/draft/application/UI impact: 26 application/draft/review checks, two
    draft-resume widgets, and seven isolated review-screen widgets passed (35
    total). Tier 3 was not due. Remaining risk: the review-screen test is 492
    lines and the broad tracking test is 498 lines; both require a descriptive
    split before expansion. Total maintenance receipt checks remain 215 before
    and after this pass; the latest full checkpoint remains Pass 62's 213-check
    gate.
73. Near-limit widget-test support extraction. Navigation/receipt fixtures and
    storage stubs moved from the registered review-screen test into
    `maintenance_receipt_review_widget_test_support.dart`; maintenance screen
    pumping and layout assertions moved from the registered tracking test into
    `maintenance_tracking_widget_test_support.dart`. Test functions and harness
    entrypoints stayed in their original files, and the shareable QA harness
    was not changed. The review-screen test fell from 492 to 396 lines and the
    tracking test from 498 to 469; support files are 115 and 38 lines with
    descriptive names. Production files changed: none. Existing tests changed
    only by imported helper aliases; no test was added, removed, or
    consolidated. Changed-file analysis and all seven review-screen plus nine
    tracking widgets passed (16 total). Tier 2 and Tier 3 were not required
    because no production/interface/state/persistence/navigation/platform
    behavior changed. Remaining risk: the roadmap/pass log are approaching the
    500-line limit and need a planned archive/index split before further
    growth. Total maintenance receipt checks remain 215 before and after this
    pass; the latest full checkpoint remains Pass 62's 213-check gate.

## Current counter

- Last accepted pass: **73**
- Next pass: **74**
