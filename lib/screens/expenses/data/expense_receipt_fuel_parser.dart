...
String? _fuelTypeSignalFor(String text) {
  ...
  if (RegExp(
    r'\b(propane|lpg|l\.?p\.?\s+gas|gas\s+l\.?p\.?|liquefied\s+petroleum\s+gas|autogas|auto\s+gas|hd[-\s]?5)\b',
  ).hasMatch(text)) {
    return 'Propane';
  }
  ...
  if (RegExp(
    r'\b(gasoline|gasohol|unleaded|regular|mid[-\s]?grade|plus|premium)\b',
  ).hasMatch(text)) {
    return 'Gasoline';
  }
  return null;
...
String? _dieselRoadClassificationFor(List<String> receiptRows) {
  ...
  final hasOffRoad = RegExp(
    r'\b(off[-\s]?road|dyed\s+diesel|red(?:\s+dyed?)?\s+diesel|diesel\s+rojo|diesel\s+te[nñ]ido|farm\s+diesel|ag\s+diesel)\b',
  ).hasMatch(text);
  final hasOnRoad = RegExp(
    r'\b(on[-\s]?road|highway|clear\s+diesel|diesel\s+(?:de|para)\s+carretera|diesel\s+claro)\b',
  ).hasMatch(text);
...
String? _fuelOctaneFor(List<String> receiptRows) {
  final gasolineGrade = RegExp(
    r'\b(?:reg|regular|unleaded|unl|mid|mid[-\s]?grade|plus|prem|premium|super|supreme|suprema|gasohol|'
