...
Object? _sanitizeExpenseTelemetryValue(String key, Object? value) {
  ...
  if (value is String) {
    ...
    if (key.endsWith('Label') || key == 'recommendedAction') {
      return _safeReadableText(value);
    }
  ...
  if (value is Map) {
    ...
    for (final entry in value.entries) {
      final countKey = _safeToken(entry.key.toString());
...
String _safeReadableText(String value) {
  final safe = value
      .trim()
