...
extension _ReceiptAttachmentNativeSignalDocuments
    on _SharedReceiptAttachmentPanelState {
  List<String> nativeCameraUiDocumentSignalsFor(
    ...
    return List.unmodifiable({
      ...
      'native_camera_ui_${attachmentSignalToken(result.nativeCameraUiHealthOutcome)}',
      for (final entry in counts.entries)
        'native_camera_ui_${attachmentSignalToken(entry.key)}',
    });
  }

  List<String> nativeCameraUiRiskFlagsFor(ReceiptPhotoReviewResult result) {
    final counts = result.nativeCameraUiHealthCounts;
    if (counts.isEmpty) return const [];
    return List.unmodifiable({
      for (final entry in counts.entries)
  ...
  List<String> nativeCloseCapturedPhotoDocumentSignalsFor(
    ReceiptPhotoReviewResult result,
  ) {
    ...
    return List.unmodifiable({
      ...
      'native_close_captured_photo_${attachmentSignalToken(result.nativeCloseCapturedPhotoHealthOutcome)}',
      for (final entry in counts.entries)
        'native_close_captured_photo_${attachmentSignalToken(entry.key)}',
    });
  }

  List<String> nativeCloseCapturedPhotoRiskFlagsFor(
    ReceiptPhotoReviewResult result,
  ) {
    final counts = result.nativeCloseCapturedPhotoOutcomeCounts;
    if (counts.isEmpty) return const [];
  ...
  List<String> nativeCaptureSourcePolicyDocumentSignalsFor(
    ReceiptPhotoReviewResult result,
  ) {
    ...
    return List.unmodifiable({
      ...
      'native_capture_source_${attachmentSignalToken(result.nativeCaptureSourcePolicyOutcome)}',
      if ((counts['existing_photo_import'] ?? 0) > 0)
        'receipt_existing_photo_import',
      for (final entry in counts.entries)
        'native_capture_source_${attachmentSignalToken(entry.key)}',
    });
  }

  List<String> nativeReceiptCameraSurfaceDocumentSignalsFor(
    ReceiptPhotoReviewResult result,
  ) {
    ...
    return List.unmodifiable({
      ...
      if (result.nativeReceiptCameraSurfaceOutcome != 'unknown')
        'native_capture_surface_${attachmentSignalToken(result.nativeReceiptCameraSurfaceOutcome)}',
      if (result.nativeReceiptCameraSurfaceVerificationOutcome != 'unknown')
        'native_capture_surface_verified_${attachmentSignalToken(result.nativeReceiptCameraSurfaceVerificationOutcome)}',
      if (result.nativeCameraIdentityOutcome != 'unknown')
        'native_capture_identity_${attachmentSignalToken(result.nativeCameraIdentityOutcome)}',
      for (final entry in surfaceCounts.entries)
        'native_capture_surface_${attachmentSignalToken(entry.key)}',
      for (final entry in verificationCounts.entries)
        'native_capture_surface_verified_${attachmentSignalToken(entry.key)}',
      for (final entry in identityCounts.entries)
        'native_capture_identity_${attachmentSignalToken(entry.key)}',
    });
  }

  List<String> nativeCaptureSourcePolicyRiskFlagsFor(
    ReceiptPhotoReviewResult result,
  ) {
    final counts = result.nativeCaptureSourcePolicyCounts;
    if (counts.isEmpty) return const [];
  ...
  List<String> receiptBrainDocumentSignalsFor(ReceiptPhotoReviewResult result) {
    ...
    return List.unmodifiable({
      ...
      if (result.receiptBrainReleaseActionOutcome !=
        ...
        'receipt_brain_release_${attachmentSignalToken(result.receiptBrainReleaseActionOutcome)}',
      if (result.receiptBrainInstallDistributionOutcome !=
          'receipt_brain_install_unknown')
        'receipt_brain_install_${attachmentSignalToken(result.receiptBrainInstallDistributionOutcome)}',
      for (final entry in releaseCounts.entries)
        'receipt_brain_release_${attachmentSignalToken(entry.key)}',
      for (final entry in installCounts.entries)
        'receipt_brain_install_${attachmentSignalToken(entry.key)}',
      for (final entry in storageCounts.entries)
        'receipt_brain_storage_${attachmentSignalToken(entry.key)}',
      for (final entry in localOcrCounts.entries)
        'receipt_brain_local_ocr_${attachmentSignalToken(entry.key)}',
    });
  }

  List<String> receiptBrainRiskFlagsFor(ReceiptPhotoReviewResult result) {
    final releaseCounts = result.receiptBrainReleaseActionCounts;
    final installCounts = result.receiptBrainInstallDistributionCounts;
    final storageCounts = result.receiptBrainStorageClassCounts;
    final localOcrCounts = result.receiptBrainLocalOcrModeCounts;
    ...
    final defersOptionalPacks =
        (releaseCounts['ship_lean_base_and_defer_optional_receipt_packs'] ??
            0) >
        0;
    ...
    return List.unmodifiable({
      ...
      if (defersOptionalPacks)
        'ocr_source_receipt_brain_optional_packs_deferred',
