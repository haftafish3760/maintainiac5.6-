...
Future<bool> confirmSuspiciousOdometerReading(
...
Future<OdometerMileageReview?> collectOdometerMileageReview(
  BuildContext context,
  int deltaMiles,
) async {
                    ...
                    Navigator.of(context).pop(
                      OdometerMileageReview(
                        ...
                        businessMiles: selectedUse == OdometerMileageUse.split
                            ? int.tryParse(businessMilesController.text.trim())
                            : null,
                      ),
                    );
                  },
...
Future<OdometerCorrectionReview?> collectOdometerCorrectionReview(
  BuildContext context, {
  required int currentReading,
  required int candidateReading,
}) async {
  OdometerCorrectionReason? selectedReason;
  return showDialog<OdometerCorrectionReview>(
    context: context,
    builder: (context) => StatefulBuilder(
      builder: (context, setDialogState) => AlertDialog(
        backgroundColor: const Color(0xFF101719),
