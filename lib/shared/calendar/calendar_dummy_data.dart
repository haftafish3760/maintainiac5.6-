...
part 'calendar_contractor_dummy_data.dart';
part 'calendar_employee_dummy_data.dart';
  ...
  if (source == CalendarFlowSource.contractor) {
    return CalendarDayData(
      recapItems: switch (mode) {
        CalendarDayMode.future => const [],
        CalendarDayMode.past || CalendarDayMode.today => const [
          CalendarRecapItem(label: 'Billed', value: r'$2,450'),
          CalendarRecapItem(label: 'Costs', value: r'$780'),
          CalendarRecapItem(label: 'Jobs', value: '3'),
          CalendarRecapItem(label: 'Miles', value: '96'),
        ],
      },
      entries: switch (mode) {
        CalendarDayMode.past => _contractorEntries(normalized),
        CalendarDayMode.today => _contractorTodayEntries(normalized),
        CalendarDayMode.future => _contractorFutureEntries(normalized),
      },
    );
  }
  ...
  if (source == CalendarFlowSource.employee) {
    return CalendarDayData(
      recapItems: switch (mode) {
        CalendarDayMode.future => const [],
        CalendarDayMode.past || CalendarDayMode.today => const [
          CalendarRecapItem(label: 'Hours', value: '7.5'),
          CalendarRecapItem(label: 'Jobs', value: '2'),
          CalendarRecapItem(label: 'Expenses', value: r'$86'),
          CalendarRecapItem(label: 'Miles', value: '42'),
        ],
      },
      entries: switch (mode) {
        CalendarDayMode.past => _employeeEntries(normalized),
        CalendarDayMode.today => _employeeTodayEntries(normalized),
        CalendarDayMode.future => _employeeFutureEntries(normalized),
      },
    );
  }
...
List<CalendarTimelineEntry> _pastEntries(DateTime day) {
  return [
    ...
    _entry(
      ...
      source: 'Trip log',
      summary: 'Work supplies picked up for invoice draft',
      details: [
        'Linked item: Invoice draft #1042',
        'Pickup location: Supplier counter',
        'Work supplies count: 4',
      ],
    ),
    _entry(
      day: day,
