...
Future<List<File>> _writeTightVerticalEdgeCroppedPhoneWindows(
  String prefix,
) async {
  final tallReceipt = tallReceiptStitchingCanvas(sectionCount: 4);
  final starts = <int>[0, 1260, 2520, 3780];
  final files = <File>[];
  for (var index = 0; index < starts.length; index++) {
    final window = img.copyCrop(
      tallReceipt,
    ...
    final clipped = index.isEven
        ? clipReceiptStitchingVerticalEdge(window, bottom: 42)
        : clipReceiptStitchingVerticalEdge(window, top: 46);
    final shifted = shiftReceiptStitchingShot(
      clipped,
