...
void main() {
  ...
  test('changed-route coverage gate requires real-device snapshot routing', () async {
    final dir = await Directory.systemTemp.createTemp(
      'receipt_changed_route_gate_',
    );
    addTearDown(() => dir.delete(recursive: true));
    ...
    final fastGuard = File('${dir.path}/fast_guard.sh');
    changedGate.writeAsStringSync(
      File('tool/receipt_camera_changed_gate.sh').readAsStringSync().replaceFirst(
    ...
    expect(
      result.stderr.toString(),
      contains(
        'MISSING_CHANGED_GATE_ROUTE tool/receipt_camera_real_device_snapshot.sh',
      ),
    );
    expect(
      result.stderr.toString(),
      contains(
        'MISSING_FAST_GUARD_ROUTE_SMOKE tool/receipt_camera_real_device_snapshot.sh',
      ),
    );
  });
}
