...
void main() {
  test(
    'real-device result start script creates a structured run note',
    () async {
      ...
      expect(text, contains('## Required Target Evidence Matrix'));
      expect(text, contains('- Galaxy S24 Ultra flagship: NOT RUN'));
      expect(text, contains('- Galaxy S25 Ultra additional flagship: NOT RUN'));
      expect(text, contains('- Galaxy S9 Plus older flagship: NOT RUN'));
      expect(text, contains('- Constrained Android emulator: NOT RUN'));
      expect(text, contains('- iPhone SE third generation: NOT RUN'));
      expect(text, contains('- Future budget Android: NOT RUN'));
      ...
      expect(text, contains('- Flutter devices snapshot: captured;'));
      expect(text, contains('- Xcode devices snapshot: captured;'));
      expect(
        text,
        isNot(
          contains('- Xcode devices snapshot: captured; inspect linked log `'),
        ),
      );
      ...
      expect(
        RegExp(
          r'^- Flutter devices snapshot log:',
          multiLine: true,
        ).allMatches(text).length,
        1,
      );
  ...
  test(
    'real-device result start script stays metadata-only and non-interactive',
    () {
      final script = File(
        'tool/receipt_real_device_result_start.sh',
      ).readAsStringSync();
      ...
      expect(
        script,
        contains(
          'Creates a privacy-safe real-device receipt-camera result note',
        ),
      );
      expect(
        script,
        contains(
          'This script records metadata only. It does not install, launch, tap,',
        ),
      );
