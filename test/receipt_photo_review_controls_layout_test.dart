...
void main() {
  ...
  test('photo review bottom controls stay capped by mode', () async {
    ...
    expect(reviewScreen, isNot(contains('phonePreviewControlsHeight = 100.0')));
    expect(
      reviewScreen,
      contains('widget.uiConfig.previewControlsSinglePhotoHeight'),
    );
    expect(
      reviewScreen,
    ...
    expect(reviewScreen, contains('targetWidth.clamp(900, 1800)'));
    expect(reviewScreen, contains('_reviewPreviewCacheHeight(context)'));
    expect(
      reviewScreen,
      contains('cacheHeight: _reviewPreviewCacheHeight(context)'),
    );
    ...
    expect(orderControls, contains('constraints.maxWidth < 390'));
    expect(
      orderControls,
      contains('MediaQuery.textScalerOf(context).scale(1) > 1.2'),
    );
    ...
    expect(previewControls, isNot(contains('height: 32')));
    expect(previewControls, contains('class _ReceiptPreviewSecondaryAction'));
    expect(previewControls, contains("compact ? 'Arrange' : 'Arrange Photos'"));
    expect(
      previewControls,
      contains('minimumSize: Size.fromHeight(compact ? 46 : 50)'),
    );
    expect(
      previewControls,
  ...
  test('photo review tray uses explicit long-receipt language', () async {
    ...
    expect(previewControls, contains('const SizedBox(height: 6)'));
    ...
    expect(previewControls, contains('width: double.infinity'));
    expect(
      previewControls,
    ...
    expect(previewControls, contains('semanticLabel: addPhotoTooltip'));
    expect(previewControls, contains('semanticLabel: retakeSemanticLabel'));
    expect(previewControls, contains("savingPhotos ? 'Opening'"));
    expect(
      previewControls,
    ...
    expect(controls, isNot(contains('onMatch: interactionLocked')));
    expect(controls, contains('onCrop: interactionLocked'));
    expect(controls, isNot(contains('_ReceiptOrderThumbnail')));
    ...
    expect(previewControls, isNot(contains('_ReceiptMultiPhotoActionRail')));
    expect(previewControls, isNot(contains("label: 'Check Photo Order'")));
    expect(previewControls, contains("semanticLabel: 'Crop receipt photo'"));
    ...
    expect(modeControls, contains('Align Photos'));
    ...
    expect(controls, contains('askSavedProofSizeEachReceipt'));
