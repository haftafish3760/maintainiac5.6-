...
void main() {
  test(
    'receipt camera footprint audit exposes machine-readable source and artifacts',
    () async {
      ...
      final iosCandidateStatuses =
          artifacts['iosInstallCandidateStatus']! as List<Object?>;
      ...
      expect(
        report['excludes'],
        contains('compiled_packaging_overhead_from_source_total'),
      );
      expect(source['totalBytes'], greaterThan(0));
      expect(source['thresholdStatus'], 'ok');
      expect(
        source['totalBytes'] as int,
        lessThan(source['reviewThresholdBytes'] as int),
      );
