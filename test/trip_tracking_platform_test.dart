...
void main() {
  ...
  test('malformed status payloads fail closed without platform details', () {
    ...
    final allowedPaused = TripTrackingPlatformEvent.fromMap({
      'type': 'status',
      'status': 'paused',
    });
    ...
    expect(allowedPaused.type, TripTrackingPlatformEventType.status);
    expect(allowedPaused.status, 'paused');
    expect(allowedPaused.errorCode, isNull);
  });
  ...
  test('impossible native speed accuracy fails closed at the boundary', () {
    final event = TripTrackingPlatformEvent.fromMap({
      'type': 'location',
      'latitude': 35.2,
      'longitude': -80.8,
